...
async function handleBirthdayCommand(message, args, prefix, getGuildData, saveData) {
  ...
  if (sub === "set") {
    ...
    const COOLDOWN_MS = 14 * 24 * 60 * 60 * 1000; // 2 weeks cooldown
  ...
  if (sub === "closest" || sub === "list") {
    ...
    const generateButtons = (page) => {
      return new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("prev_page").setLabel("⬅️ Prev").setStyle(ButtonStyle.Primary).setDisabled(page === 0),
        new ButtonBuilder().setCustomId("next_page").setLabel("Next ➡️").setStyle(ButtonStyle.Primary).setDisabled(page === totalPages - 1)
      );
    };
